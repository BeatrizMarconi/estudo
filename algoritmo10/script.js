// let nota_1= 10;
// let nota_2= 5;
// let nota_3= 7;
// let nota_4= 7;
// let media= (nota_1 + nota_2 + nota_3 + nota_4)/4;

// if(media >= 6){
//     console.log(`APROVADO, sua média é: ${media}`)
// }else{
//     console.log(`REPROVADO, sua média é: ${media}`)
// }

let nota_1= 10;
let nota_2= 5;
let nota_3= 7;
let nota_4= 7;
let media= (nota_1 + nota_2 + nota_3 + nota_4)/4;

if(media >= 6){
    console.log(`Aluno aprovado com média ${media}`)
}
else if(media >= 5 && media <= 5.9){
    console.log(`Aluno será encaminhado para conselho de classe`)
}
else{
    console.log(`Aluno reprovado com média ${media}`)
}



// let num1=15;
// let num2=35;
// let res= num1 + num2;

// console.log(`O resultado da soma das variáveis é: ${res}`)


// let nome = 'cheová';
// let sobrenome = 'marconi';
// let idade = 34;

// console.log(`Olá! Meu nome é ${nome} ${sobrenome} e eu tenho ${idade} anos`)

